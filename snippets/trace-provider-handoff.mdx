# Trace Provider Data Audit — Staging Handoff & API Reference
...
`initial_metadata_root` should be the provider's deterministic non-zero hash of the canonical Trace Schema v1.0 metadata JSON. In the current staging API, Story stores this value as submitted; hash verification against `initial_metadata_json` is not enforced yet.
