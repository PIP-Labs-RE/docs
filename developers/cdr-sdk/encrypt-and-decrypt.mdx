...
<Frame>
  <img
    src="/images/developers/cdr-sdk/cdr-encryption-flow.png"
...
<Frame>
  <img
    src="/images/developers/cdr-sdk/cdr-decryption-flow.png"
...
<Note>
  In `v0.1.1`, `accessCDR()` can generate the ephemeral keypair and query the
  threshold for you when those params are omitted. This guide passes them
  explicitly so the underlying decryption flow stays visible.
</Note>
...
<Note>
  If you use `HeliaProvider`, pass the `CID.parse` function into the
  constructor as shown above to avoid class mismatches.
</Note>
