---
title: Full Node
description: Guide to setting up a DATA Foundation node for mainnet
---
...
The `story` and `geth` binaries, which make up the clients required for running DATA Foundation nodes, are available from our latest `release` pages:
...
- Network: DATA Foundation Mainnet or DATA Foundation Aeneid Testnet
- Required versions:
...
<Note>The DATA Foundation API endpoint (`--api-address`) can be modified as needed depending on your environment or use case.</Note>
