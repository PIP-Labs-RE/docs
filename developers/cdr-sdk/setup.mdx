...
<Note>
  The `network` parameter determines which contract addresses the SDK uses. When
  pointing to a custom RPC, choose the `network` value that matches the contract
  deployment on that chain. Both `"testnet"` and `"mainnet"` currently use the
  same contract addresses, but this may change in the future.
</Note>
