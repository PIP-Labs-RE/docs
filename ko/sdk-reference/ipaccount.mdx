---
title: IP Account
description: IPAccountClient를 사용하면 IP Account 메타데이터를 관리하고 트랜잭션을 실행할 수 있습니다.
---
...
IP asset에 대한 metadataURI를 설정합니다.
...
- `request.metadataURI`: IP asset에 대해 설정할 metadataURI. [IPA Metadata Standard](/concepts/ip-asset/ipa-metadata-standard)에 맞는 메타데이터를 가리키는 URL이어야 합니다.
...
- `request.ipId`: ip account를 얻을 Ip Id.
- `request.to`: 트랜잭션의 수신자.
- `request.value`: 보낼 Ether의 양.
...
- `request.signature`: 트랜잭션의 서명, EIP-712 인코딩.
- `request.value`: \[선택] 보낼 Ether의 양.
...
- `request.tokens`: 전송할 토큰 정보
  - `request.tokens.address`: WIP 및 표준 ERC20을 포함한 ERC20 토큰의 주소.
