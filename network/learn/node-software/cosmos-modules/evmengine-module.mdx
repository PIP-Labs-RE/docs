...
This document specifies the internal `x/evmengine` module of the Data Foundation Network.

As the Data Foundation Network separates the consensus and execution client, like Ethereum, the consensus client (CL) and execution client (EL) needs to communicate to sync to the network, propose proper EVM blocks, and execute EVM-triggered EL actions in CL.
