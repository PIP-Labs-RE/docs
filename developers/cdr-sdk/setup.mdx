...
```bash Build SDK
git clone https://github.com/piplabs/cdr-sdk.git --branch 0.1.1
cd cdr-sdk
pnpm install
pnpm build
```

```bash Install in your app
npm install viem
npm install --save <path-to-cdr-sdk>/packages/sdk
npm install --save <path-to-cdr-sdk>/packages/contracts
npm install --save <path-to-cdr-sdk>/packages/crypto
```
...
- For DKG backends, validation RPCs, system addresses, and current Aeneid
  runtime notes, see
