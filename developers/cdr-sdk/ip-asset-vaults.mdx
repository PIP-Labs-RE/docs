...
CDR vaults can be gated behind the DATA Foundation's license tokens so that only
...
mint DATA Foundation license tokens separately.
...
Create the IP and obtain its `ipId` with DATA Foundation tooling first, then configure
the vault.
...
Before a user can read a DATA Foundation license-gated vault, they still need to mint a
license token. The DATA Foundation core SDK's `wipClient` handles the WIP wrap and
