...
| 4 | Registered | Root-level `occurred_at` of the `DataRegistered` event (`seq: 0`) |
...
There are two `occurred_at` locations, both provider-supplied: the root field on each batch item is the required one and dates the event; `timestamps.occurred_at` carries the same moment inside the attested payload — keep them equal. The only values the DATA Foundation adds are `ingested_at` and `tx_hash`.
...
- `GET /data-ids/{data_id}` returns the registration profile plus the latest raw metadata event — the exact payload of the highest stored sequence, not necessarily a diff.
...
Search is exact-match on the [searchable fields](#searchable-fields); `source_record_id` search is exact and case-sensitive. Each match includes `tx_hash` (initially an empty string), and responses include `next_cursor` for pagination. `limit` defaults to `100` and is capped at `1000` — larger values are silently clamped, not rejected.
