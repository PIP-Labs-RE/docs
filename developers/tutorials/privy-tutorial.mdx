...
4. Once you enable Smart wallets, right underneath make sure to put a "Custom chain" with the following values:
   1. Name: `Aeneid Testnet`
   2. ID number: `1315`
   3. RPC URL: `https://aeneid.datarpc.io`
   4. For the Bundler URL and Paymaster URL, go to [Pimlico's Dashboard](https://dashboard.pimlico.io) and create a new app. Then click on "API Keys", create a new API Key, click "RPC URLs" as shown below, and then select "Aeneid Testnet" as the network:
...
## 5. Send a Transaction From Data Foundation SDK
