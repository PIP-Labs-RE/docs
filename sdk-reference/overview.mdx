...
The CDR SDK (`@piplabs/cdr-sdk`) provides a TypeScript client for Story's Confidential Data Rails system. Use it to encrypt secrets using threshold cryptography, store them in on-chain vaults with configurable access control, and decrypt them by collecting partial decryptions from validators.
