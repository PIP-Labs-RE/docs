...
쓰기 테스트에는 제공된 스테이징 API 키를 사용하세요. API 키는 해당 프로바이더와 연결되어 있어야 하며, 쓰기 요청에서 `X-Provider`는 그 프로바이더와 일치해야 합니다. 읽기/검색/스코프 그룹 엔드포인트는 공개 감사 뷰입니다.

읽기 API는 글로벌 Story `data_id`를 키로 사용합니다. 프로바이더는 필드로 반환되며 선택적 필터 또는 검색 가능한 필드로 사용할 수 있습니다.

## 현재 프로바이더 / Trace 정렬

프로바이더의 초안 공개 페이로드는 현재 Trace Schema v1.0 워킹 셰이프(working shape)로 깔끔하게 매핑됩니다. 프로바이더는 아래 표시된 정규화된 Trace 필드를 전송하고, 프로바이더별 세부 정보가 손실되지 않도록 전체 프로바이더 공개 페이로드를 `provider_payload` 아래에 포함해야 합니다.

다음 필드들은 현재 Trace v1.0 워킹 셰이프의 일부입니다:

- `contributor.kyc_country`: 국가 단위의 KYC 관할권 신호. 국가만 포함되며, 주소나 GPS에서 파생된 국가는 포함되지 않습니다.
- `contributor.consent.tos_*` 및 `contributor.consent.privacy_policy_*`: 해당 레코드에 대해 컨트리뷰터가 수락한 정확한 정책 버전과 해시.
- `attestation.signed_at_utc` 및 `attestation.key_url`: 최상위 `attestation` 블록의 검증 메타데이터.
- `file.behavior`: PII가 아닌 캡처/업로드 동작 신호.
- `file_specific.base.motion`: 미디어 유형 전반에 걸친 공유 모션 신호.
- `app.legal_entity`: 앱/플랫폼 이름과 함께 제공되는 법적 계약 상대방 정보.

프로바이더 수준의 활성 ToS 및 개인정보처리방침은 각 레코드 페이로드 내부가 아니라 프로바이더 정책 엔드포인트를 통해 설정합니다. Trace는 활성 정책의 버전, 해시, URI를 저장하므로, 레코드와 통계에서 수락된 정책 참조를 프로바이더의 현재 정책 문서와 연결할 수 있습니다.

다음 필드는 페이로드에 저장되지만 현재 스테이징 구현에서는 인덱싱되지 않습니다:
...
현재 정확 일치(exact-match) 인덱스는 Trace 프론트엔드 및 감사 플로우에 필요한 필드를 다룹니다. 더 풍부한 프로바이더 필드들도 여전히 저장되므로, 프로바이더에게 과거 데이터를 재전송하도록 요청하지 않고도 나중에 노출하거나 인덱싱할 수 있습니다.

현재 스테이징 동작:

- 메타데이터 업데이트 상한은 이제 스테이징에서 `data_id`당 `100`입니다. 동일한 프로바이더 측 리비전의 일부인 여러 변경 가능한 필드 변경들은 하나의 `seq`를 가진 하나의 메타데이터 업데이트로 병합될 수 있습니다.
- 현재 스테이징 빌드에서 읽기/검색/스코프 그룹 API는 공개 감사 뷰입니다. 엔터프라이즈 전용 또는 민감한 필드를 공개 Trace 페이로드에 넣지 마세요. 향후 프로바이더 전용 또는 파트너 전용 읽기 계층이 필요하다면 별도의 API/제품 결정이 되어야 합니다.
- 유일하게 검색 가능한 해시 유형은 정규(canonical) 콘텐츠 SHA-256입니다. `phash64`, `dhash64`, `ahash64`, `keyframe_phashes`는 유용할 때 여전히 전송해야 하지만, 현재는 저장 전용입니다.
...
- 어테스테이션(attestation) 서명은 스테이징에서 선택 사항입니다. 프로덕션 검증을 위해서는 프로바이더가 `payload_hash`, `signature`, `key_id`, `key_url`, `signed_at_utc`를 전송해야 합니다.
- `source_record_id`는 프로바이더의 안정적인 공개 미디어 ID를 포함해야 합니다(예: `kmf_...`). 스테이징은 주변 공백을 트리밍하고, 대소문자를 보존하며, 제어 문자는 거부하고, 최대 512바이트까지의 source ID를 허용합니다.

## Trace Schema v1.0

Story는 프로바이더의 원본 페이로드를 최상위 Story 계약으로 취급하는 대신 프로바이더 정규화된 trace 메타데이터 객체를 저장합니다. 프로바이더의 전체 원본 공개 페이로드는 `provider_payload` 아래에 보존되어야 합니다.

프로바이더는 표준화된 Trace Schema 필드를 직접 채워야 합니다. Story는 또한 프로바이더의 전체 원본 공개 페이로드를 `provider_payload` 아래에 보존하지만, 정규화된 필드가 다른 프로바이더와 프론트엔드가 사용해야 할 이식 가능한(portable) Trace 계약입니다.

`initial_metadata_json`과 `metadata_json`에서 `schema_version: trace-v1.0`을 사용하세요. Story는 내부 이벤트 해시를 계산하기 전에 메타데이터 JSON을 정규화(canonicalize)하므로, 객체 키 순서는 멱등성이나 충돌 감지에 영향을 주지 않습니다.

프로바이더는 쓰기 페이로드에 트랜잭션 해시를 전송할 필요가 없습니다. `tx_hash`는 Story가 소유하며, 등록, 메타데이터 업데이트, 검색 결과 행에 대한 읽기 응답에서 반환합니다. 브로드캐스트 후 Story가 채우기 전까지는 빈 문자열로 반환됩니다.

초기 등록에는 하나의 정규(canonical) 콘텐츠 해시가 반드시 포함되어야 합니다. 허용되는 필드는
`asset.hash`, `content_hash`, `file.content_sha256`, `file.hashes.sha256`이며,
모두 `sha256:<64-lowercase-hex>`로 정규화됩니다. 둘 이상의 별칭이 존재하는
경우, 그것들은 반드시 같은 해시를 나타내야 합니다.
...
`app.platform_name`, `app.legal_entity`, `file.behavior`, `file_specific.base.motion`, `file.hashes.phash64`, `dhash64`, `ahash64`, `md5`, `keyframe_phashes` 같은 필드는 저장된 페이로드에 보존될 수 있지만, 현재 스테이징 구현에서는 정확 일치로 인덱싱되지 않습니다. 분포에는 `/stats`를 사용하고, Trace 프론트엔드/감사 뷰에 대한 선택적 쿼리 스코프로 `provider`를 사용하세요.

### 필드 가이드

- `source_record_id`: 프로바이더 소유의 안정적인 공개 미디어 ID. 프로바이더의 `kmf_...` 값이 여기에 해당합니다. 스테이징은 이 값을 최대 512바이트까지 허용합니다.
- `contributor.anon_id`: 프로바이더 소유의 공개 익명화 컨트리뷰터 ID.
- `contributor.kyc_status`: 권장 값은 `verified`, `pending`, `failed`, `unverified`입니다.
- `contributor.kyc_country`: 가능한 경우 KYC의 ISO 3166-1 alpha-2 국가 코드. 국가만 포함되며, 주소나 GPS에서 파생된 국가는 포함되지 않습니다.
- `contributor.tax_status`: 권장 값은 `submitted`, `not_submitted`, `not_applicable`, `unknown`입니다. 프로바이더의 `tax_form_on_file`의 경우 `true`는 `submitted`로, `false`는 `not_submitted`로 매핑합니다.
- `contributor.account_verification_status`: 권장 값은 `verified`, `pending`, `failed`, `unverified`입니다.
- `contributor.consent.tos_*` 및 `contributor.consent.privacy_policy_*`: 이 레코드에 대해 수락된 정책 버전, 해시, URI.
- 프로바이더 활성 정책은 `PUT /webhook/v1/data-audit/provider-policy`를 통해 별도로 설정됩니다.
- `attestation.signature`: 스테이징에서 선택 사항입니다. 프로덕션 검증을 위해 프로바이더는 `payload_hash`, `signature`, `key_id`, `key_url`, `signed_at_utc`를 전송해야 합니다.
- `file.behavior`: PII가 아닌 업로드/캡처 동작 신호.
- `file_specific.base.motion`: 미디어 종류 전반에 적용 가능한 공유 모션 신호.

## 쓰기 API

### 활성 프로바이더 정책 설정

프로바이더가 새로운 현재 이용약관 또는 개인정보처리방침을 게시할 때 이 엔드포인트를 사용하세요. Trace는 활성 버전, SHA-256 해시, URI를 저장하므로, 프론트엔드는 정책 문서로 링크할 수 있고 통계는 레코드 수준의 수락된 정책 참조를 프로바이더의 현재 정책과 비교할 수 있습니다.

정책 문서 행은 감사/가시성 목적으로만 저장됩니다. 그것들은 직접 인덱싱되거나 집계되지 않습니다. 레코드 페이로드에는 여전히 `contributor.consent.tos_*`와 `contributor.consent.privacy_policy_*`가 포함되어야 하며, `/stats`, 스코프 그룹 요약, 정책 해시 검색은 이러한 레코드 수준의 수락된 정책 참조를 사용합니다.
...
초기 백로그 및 라이브 등록 배치에 이 엔드포인트를 사용하세요. 프로바이더는 안정적인 `source_record_id`를 전송하고, Story는 `data_id`를 생성하여 매핑을 반환합니다.
...
`initial_metadata_root`는 정규(canonical) Trace Schema v1.0 메타데이터 JSON에 대한 프로바이더의 결정론적 해시여야 합니다. 현재 스테이징 API에서 Story는 이 값을 제출된 그대로 저장합니다. `initial_metadata_json`에 대한 해시 검증은 아직 강제되지 않습니다.

성공 응답:

- `202 Accepted`: 하나 이상의 레코드가 새로 수락되어 큐에 등록되었습니다.
- `200 OK`: 요청은 처리되었지만, 모든 항목이 `duplicate`였기 때문에 큐에 등록된 레코드가 없습니다.
- `409 Conflict`: 하나 이상의 항목이 `conflict`였으며, 동일한 응답에 포함된 `accepted` 항목들은 그대로 큐에 등록되었습니다.
...
반환된 `data_id`는 이후의 메타데이터 업데이트와 조회를 위한 Story ID입니다. 동일한 `X-Provider` + `source_record_id`를 다시 전송하면 동일한 `data_id`가 생성됩니다.

레코드가 이미 정확히 동일한 초기 등록 페이로드로 저장되어 있는 경우, 해당 항목은 `status: "duplicate"`를 반환하며 다시 큐에 등록되지 않습니다. 동일한 `source_record_id`가 이미 다른 초기 메타데이터로 저장되어 있는 경우, 항목은 `status: "conflict"`를 반환하며 큐에 등록되지 않습니다. 첫 번째 요청이 아직 큐에 남아 있는 동안 겹치는 재시도가 들어오면 `accepted`가 반환될 수 있으며, 다운스트림 수집은 여전히 멱등성을 유지합니다. 동일한 배치의 다른 유효한 항목들은 여전히 `accepted`를 반환할 수 있습니다.
...
이후의 정정 또는 변경 가능한 메타데이터 변경 사항에 이 엔드포인트를 사용하세요. `seq`는 동일한 `data_id`에 대해 `1`과 `100` 사이여야 합니다.

`metadata_json`은 부분적 diff나 JSON patch가 아니라, 변경 후의 전체 최신 Trace 메타데이터 상태여야 합니다. 예를 들어 KYC만 변경되더라도, 프로바이더는 여전히 현재 메타데이터 상태의 일부로 남아 있는 변경되지 않은 file, asset, app, consent, provider payload 필드를 포함해야 합니다. 이렇게 하면 각 메타데이터 이벤트가 `metadata_root`에 대해 독립적으로 검증 가능해지며, Story가 프로바이더별 병합 규칙 없이도 최신 상태를 재구성할 수 있습니다.
...
`metadata_root`는 정규(canonical) 전체 업데이트된 Trace Schema v1.0 메타데이터 JSON에 대한 프로바이더의 결정론적 해시여야 합니다. 현재 스테이징 API에서 Story는 이 값을 제출된 그대로 저장합니다. `metadata_json`에 대한 해시 검증은 아직 강제되지 않습니다.

### 선택적 백로그 파일 엔드포인트

일반적인 통합 경로는 `/records:batch`에서 `application/json`을 사용하는 것입니다. 백로그 도구를 위해 다음과 같은 하위 레벨 라우트 변형도 존재하지만, 데이터 ID 파일 라우트는 모든 레코드에 `data_id`를 요구합니다.
...
압축 해제된 각 줄은 위에 표시된 것과 동일한 필드를 가진 하나의 JSON 레코드입니다.
...
`initial_metadata_json`과 `metadata_json`은 존재할 경우 인용된 CSV 필드 내에서 유효한 JSON이어야 합니다.

TXT에는 `Content-Type: text/plain`이 필요하며, 줄바꿈으로 구분된 JSON, JSON 배열, 또는 동일한 CSV 컬럼을 사용하는 헤더로 구분된 쉼표/탭 텍스트를 허용합니다.

## 읽기 API

읽기 엔드포인트는 공개 감사 뷰입니다. `provider`는 선택 사항이며 좁히기(narrowing) 필터 역할을 합니다.

읽기 모델:

- `GET /data-ids/{data_id}`는 등록 프로필과 최신 원본 메타데이터 이벤트를 반환합니다. 최신 메타데이터 이벤트가 diff라는 보장은 없으며, 현재 저장된 가장 높은 시퀀스에 대해 제출된 정확한 업데이트 페이로드입니다.
- `GET /data-ids/{data_id}/metadatas`는 `seq: 0`의 등록과 이후의 메타데이터 업데이트를 포함한 전체 append-only 메타데이터 히스토리를 반환합니다.
- 검색, 애셋 영수증(receipt) 조회, 스코프 그룹 요약은 이러한 이벤트로부터 파생된 Story의 정규화된 최신 상태 프로젝션을 사용합니다. 이 프로젝션이 MIME 타입, 미디어 카테고리, KYC 상태, TOS/개인정보처리방침 버전, 라이프사이클 상태, `tx_hash` 같은 필드를 뒷받침합니다.

### Story data ID로 trace 조회
...
각 메타데이터 행에는 `tx_hash`가 포함되며, 처음에는 빈 문자열입니다:
...
각 검색 매치에는 `tx_hash`가 포함되며, 처음에는 빈 문자열입니다. 검색 응답에는 `next_cursor`가 포함됩니다. 정확한 필드 값으로 레코드를 찾으려면 `/search`를 사용한 다음, 감사 검증에 사용되는 정규(canonical) 이벤트 페이로드를 얻으려면 `/api/v1/data-audit/data-ids/{data_id}` 또는 `/api/v1/data-audit/data-ids/{data_id}/metadatas`를 사용하세요. MIME 타입, 미디어 카테고리, KYC 상태, 국가/지역, TOS, 개인정보처리방침 버전 등 폭넓은 분포 카운트에는 `/stats`를 사용하세요. UI에서 가장 새로운 레코드가 필요할 때는 `/recent` 또는 `/feed`를 사용하세요.
...
레코드 총계는 등록된 레코드를 한 번 카운트합니다. 컨트리뷰터 총계는 컨트리뷰터 ID가 있을 때 고유한 `provider + contributor_anon_id` 값을 카운트합니다. 메타데이터 업데이트는 이러한 총계를 증가시키지 않습니다. 분포 맵과 크기 총계는 최신 애셋 프로젝션을 따르므로, 전체 상태 메타데이터 업데이트가 레코드를 한 버킷에서 다른 버킷으로 이동시킬 수 있습니다. `average_size_bytes`는 `total_size_bytes / size_record_count`이며, `size_record_count`는 양수 크기를 가진 최신 프로젝션을 카운트합니다. App/플랫폼 필드는 레코드에 저장될 수 있지만, 통계 스코프는 아닙니다.
...
피드는 등록과 메타데이터 업데이트를 모두 포함하여 최근 감사 이벤트를 반환합니다. 행은 수집 시간 기준 최신 순이며 `event_type`, `seq`, `data_id`, `source_record_id`, `asset_hash`, `occurred_at`, `ingested_at`을 포함합니다. 이전 행을 가져오려면 `next_cursor`를 사용하세요.
...
이는 등록된 영수증 행만 수집 시간 기준 최신 순으로 반환합니다. 이전 행을 가져오려면 `next_cursor`를 사용하세요.
...
허용되는 해시 형식은 `sha256:<64-hex>`, 순수 `<64-hex>`, `0x<64-hex>`, `0x1220<64-hex>`입니다. 응답은 정규(canonical) `sha256:<64-lowercase-hex>` 애셋 해시와 매치된 모든 영수증 행을 반환합니다.

### 스코프 그룹

스코프 그룹은 Trace 프론트엔드나 파트너 리뷰어가 리뷰 세트에 대한 공개 스냅샷을 생성할 수 있게 해줍니다. 프로바이더 리뷰 워크플로우의 경우, 랩(lab)은 Story가 생성한 `data_id`나 콘텐츠 해시가 아닌 프로바이더의 `source_record_id` 값을 사용해야 합니다. Trace는 결정론적 `data_id`를 계산하고, 제출된 모든 레코드의 존재를 확인하며, 전체 세트가 유효할 때만 그룹을 생성합니다.
...
source-record 그룹은 전부 아니면 전무(all-or-nothing)입니다. 중복된 `provider + source_record_id` 값은 `400`을 반환하고, 없는 레코드는 `404`를 반환하며, 프로필/source 불일치는 `409`를 반환하고, 애셋 프로젝션 없이 등록된 레코드는 `422`를 반환합니다.

더 큰 source-record CSV/TXT 입력의 경우, 먼저 사전 서명된(presigned) 업로드 URL을 요청하세요:
...
한 컬럼 CSV는 `source_record_id`를 사용하며, 생성 요청에 `provider`가 필요합니다:
...
다중 프로바이더 CSV는 두 컬럼을 모두 포함하며 최상위 `provider`가 필요하지 않습니다:
...
반환된 `Content-Type` 헤더로 반환된 `upload_url`에 파일 바이트를 업로드한 다음, 다음과 같이 그룹을 생성하세요:
...
`GET /scoped-groups/{group_id}`는 그룹 상태를 반환하며, 완료된 후에는 `summary`에 집계 지표를 반환합니다:
...
그룹이 여전히 큐에 있거나 처리 중이면 `profile.status`는 `pending` 또는 `processing`이며 `summary`는 생략됩니다.

`/items`는 제출된 각 `source_record_id`에 대해 해결된 `data_id`, `asset_hash`, 영수증 요약과 함께 한 행씩 반환합니다. 커서 기반 페이지네이션이 적용되며, 더 많은 행이 있을 때 `next_cursor`를 반환합니다. CSV export는 `input_type,provider,source_record_id,data_id,asset_hash,status,...`로 시작합니다.
...
- `502`, `503`, `504`, 네트워크 타임아웃, `429`는 지수 백오프와 지터를 사용해 재시도하세요.
- 요청이 수정되기 전까지는 검증/인증 `4xx`를 재시도하지 마세요.
- 재시도 전반에 걸쳐 `data_id`, 요청 본문, `X-Batch-Id`를 안정적으로 유지하세요.
- 쓰기 경로는 동일한 `data_id`, 이벤트 키, 이벤트 해시에 대해 멱등적입니다.
- 동일한 `data_id`와 이벤트 키가 서로 다른 메타데이터로 재시도되는 경우, 레코드는 충돌로 취급되어 거부됩니다.
