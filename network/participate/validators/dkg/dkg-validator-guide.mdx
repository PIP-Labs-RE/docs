...
The **trusted block must be within the last 2 weeks**. The light client uses a trust period, so if the trusted block is older, header verification will fail.
...
**Start order matters.** Start story-kernel AFTER the chain is running, since the kernel needs CometBFT RPC (port 26657) for light client initialization.
