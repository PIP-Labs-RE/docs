...
<CardGroup cols={2}>
  <Card
    title="Building with an AI agent? Install the CDR Skill"
    icon="robot"
  ...
  <Card
    title="CDR Whitepaper"
    icon="file-lines"
    href="https://www.story.foundation/confidentialdatarails.pdf"
  >
    The full design — cryptography, validator protocol, and threat model — behind
...
- DKG state reads over the Data Foundation API REST endpoint (`apiUrl`)
...
uploader can write, while readers must present valid Data Foundation license token IDs in
`accessAuxData`.
...
For the deployed Data Foundation license-gated pattern, see
[How the Data Foundation License Read Pattern Works](/developers/cdr-sdk/ip-asset-vaults#how-the-data-foundation-license-read-pattern-works).
